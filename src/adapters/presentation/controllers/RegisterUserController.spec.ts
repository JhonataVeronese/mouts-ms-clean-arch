...
describe("Register User Controller", () => {
  ...
  test("should return 400 if an invalid email is provided", async () => {
    ...
    jest
      .spyOn(registerUserStub, "registerUserOnMailingList")
      .mockImplementationOnce(async (user: UserData) => {
        return await Promise.resolve(left(new InvalidEmailError(user.email)));
      });
    const httpResponse = await sut.handle(httpRequest);
    expect(httpResponse.statusCode).toBe(400);
    expect(httpResponse.body).toEqual(
      new InvalidEmailError(httpRequest.body.email).message
    );
  });
  ...
  test("should return 400 if an invalid name is provided", async () => {
    ...
    expect(httpResponse.statusCode).toBe(400);
    expect(httpResponse.body).toEqual(
      new InvalidNameError(httpRequest.body.name).message
    );
  });
  ...
  test("should return 500 if send email user throws", async () => {
    const { sut, sendEmailToUserStub } = makeSut();
    jest
      .spyOn(sendEmailToUserStub, "sendEmailToUserWithBonus")
      .mockImplementation(() => {
        throw new Error();
      });
    const httpRequest = {
